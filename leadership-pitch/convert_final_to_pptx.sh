#!/bin/bash
# Convert final markdown presentation to PowerPoint format
# Usage: ./convert_final_to_pptx.sh

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INPUT_MD="$SCRIPT_DIR/orchestrated_ai_presentation_final.md"
OUTPUT_PPTX="$SCRIPT_DIR/orchestrated_ai_presentation_final.pptx"
REFERENCE_PPTX="$SCRIPT_DIR/reference.pptx"

echo "🎯 Converting Final Markdown to PowerPoint..."
echo "   Input: $INPUT_MD"
...
if [ -f "$OUTPUT_PPTX" ]; then
    ...
    echo "   - 16 high-quality PNG diagrams"
    echo "   - Optimized fonts and layouts to prevent overflow"
    echo "   - All ASCII diagrams converted to Mermaid visuals"
    echo ""
